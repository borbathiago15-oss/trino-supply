...
{
    private CostCenter(CostCenterId id, CompanyId companyId, string code, string name) : base(id)
    {
        CompanyId = companyId;
        Code = code;
        Name = name;
    ...
    public static Result<CostCenter> Create(CompanyId companyId, string code, string name)
    {
        ...
        return Result.Success(new CostCenter(CostCenterId.New(), companyId, code.Trim().ToUpperInvariant(), name.Trim()));
    }
