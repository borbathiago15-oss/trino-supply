...
| 2026-08-27 | Painel de Atendimentos com cartões clicáveis: cada indicador (aguardando aprovação, em andamento, concluídos, parciais aguardando compra) abre a lista por trás do número — mostrando só aquele bloco e voltando ao painel completo no segundo clique. A lista de "aguardando aprovação" passou a existir na tela (o endpoint já a devolvia) e cada bloco ganhou o atalho para onde a pendência é resolvida: Central de Aprovação, Fila de Atendimento e Processos de Cotação | MMS-003, MMS-004 |
| 2026-08-29 | Engenharia de evolução do produto (Trino Supply × Procurement Management System): análise completa em `analise-evolucao-trino-supply.md` — matriz de comparação por área, 10 conflitos com recomendação (registro da O.C. mantido; base do saving única; status derivado, não persistido; agrupamento multi-SC como única alteração de regra existente, pendente de aprovação; homologação com grandfathering; códigos de erro imutáveis), backlog priorizado P1–P4, impacto em arquitetura/dados/APIs, dados por indicador (OTIF calculável com dados existentes), 9 dashboards propostos, classificação das capacidades de IA e checklist de preservação das 12 regras críticas. `prompt-sistema-trino-supply-v2.md` criado como PROPOSTA de novo prompt oficial — superconjunto do atual, vigora somente após validação | GOV-002, RFQ-001, SUP-001, PO-001, MMS-003 |
| 2026-08-29 | Prompt V2 APROVADO pelo responsável do produto (inclusive a generalização da regra 1 — cotação a partir de itens de várias SCs, a implementar na Fase 2) e Fase 1 do backlog implementada: (1) compra URGENTE exige justificativa da urgência e impacto da não compra (PR-ERR-050; campos exibidos na triagem, no processo e na edição, limpos ao voltar para NORMAL); (2) aging na Gestão de Solicitações — faixas 0-2/3-5/6-10/+10 dias com badge por item, contadores e filtro "Tempo na fila"; (3) OTIF — o registro da O.C. congela a data prometida (data da O.C. + prazo da proposta vencedora), a entrega deriva On-Time/In-Full/OTIF por pedido (badge no processo) e a tabela de fornecedores do dashboard ganhou o OTIF % consolidado. Migration `AddUrgencyJustificationAndPromisedDate` | GOV-002, PR-001, RFQ-001, PO-001 |
| 2026-08-29 | Fase 2 do V2 — entrega 1 (compras com custo total e contratos com teto): a proposta ganhou impostos e outros custos (total = itens + frete + impostos + outros − desconto, no comprador e no portal, com as linhas no mapa); o registro da O.C. congela o último preço pago e o saving de referência por item de catálogo (métrica separada do saving de negociação, exposta no pedido e somada no dashboard); o contrato de parceria ganhou teto financeiro com consumo derivado das O.C.s na vigência e saldo — exceder o teto exige justificativa (CT-ERR-010; evento CONTRATO_TETO_EXCEDIDO na timeline). Novo módulo CONTRATOS e a tela Compras → Contratos (vigência, teto, consumido, saldo, spend sob contrato). Migration `AddProposalCostsReferenceSavingContractLimit` | RFQ-001, SUP-001, PO-001 |
| 2026-08-29 | Fase 2 do V2 — entrega 2 (homologação + SLA): fornecedor ganhou o ciclo de homologação (PROSPECT/EM_HOMOLOGACAO/HOMOLOGADO/RESTRITO/BLOQUEADO — existentes e novos nascem HOMOLOGADO, grandfathering) com certidões de validade monitorada (`procurement.supplier_document`; CND Federal, FGTS, CNDT, contrato social; VENCIDA/vence em Xd na tela). Regras: certidão vencida restringe automaticamente só quem tem certidões cadastradas; PROSPECT participa e propõe, mas a seleção do vencedor exige HOMOLOGADO (SUP-ERR-030); BLOQUEADO não é convidado; homologação é do gestor de suprimentos (PATCH /suppliers/{id}/homologation, SUP-ERR-031/032). O painel de prazos do dashboard virou SLA por processo: mediana, % dentro do prazo e nº de medições por etapa. Migration `AddSupplierHomologationAndDocuments` | SUP-001, RFQ-001 |
