...
| MMS-003-02 | Material Requisition — Business Rules | 1.0.0 | 🟢 Approved | `docs/03-business/materials/MMS-003/02-business-rules.md` |
...
| MMS-003-01 | Material Requisition — Business Context | 1.0.0 | 🟢 Approved | `docs/03-business/materials/MMS-003/01-business-context.md` |
| MMS-003-06 | Material Requisition — BPMN | 1.0.0 | 🟢 Approved | `docs/03-business/materials/MMS-003/06-bpmn.md` |
| MMS-003-08 | Material Requisition — User Stories | 1.0.0 | 🟢 Approved | `docs/03-business/materials/MMS-003/08-user-stories.md` |
...
| MMS-004-03 | Inventory Management — State Machine | 1.1.0 | 🟢 Approved | `docs/03-business/materials/MMS-004/03-state-machine.md` |
...
| SEC-004 | Enforcement de Isolamento Multi-Tenant (RLS) | 1.0.0 | 🟢 Approved | `docs/08-security/04-multitenant-rls-enforcement.md` |
...
|------|--------|------------|
| 2026-08-09 | **SEC-004 — Enforcement de Isolamento Multi-Tenant (RLS) criado e aprovado (v1.0.0) + correções de segurança no esqueleto:** revisão como engenharia de segurança do código inicial (GO-001). **Corrigido bug crítico:** a `foundation.outbox` tinha RLS com `WITH CHECK` sem ninguém definir o GUC → bloqueava **todos** os INSERTs e dava falsa sensação de segurança (removido; isolamento da outbox por privilégio de role). **Implementado enforcement real do RLS:** `TenantConnectionInterceptor` (fail-closed) que define `app.current_company` por conexão, parametrizado; JWT **fail-closed** (validação estrita de assinatura/emissor/audiência/expiração; recusa iniciar em produção sem AuthN configurada); `IHasDomainEvents` para varredura correta do Outbox entre contextos; hardening de role de banco (`NOSUPERUSER NOBYPASSRLS`). SEC-004 documenta o modelo de três camadas | SEC-004, ADR-015 |
...
| 2026-08-08 | **MMS-003-11 (Database) e MMS-003-13 (API) do Material Requisition criados e aprovados (versão 1.0.0):** Database — 4 tabelas no schema `materials` (material_requisition + item + attachment, delivery_location), constraints (status ST-MR, quantidade na unidade base, rota estoque/compra, código de local gerado), refs de reserva/compra (rastreabilidade bidirecional), sem tabela de saldo (MR-BR-050). API — contrato `/api/v1/material-requisitions` com 19 endpoints (ciclo de vida, aprovação com SoD e aprovação parcial, visão do almoxarifado, locais de entrega), erros MR-ERR, segurança e roadmap. **Completa a cadeia técnica do MMS-003 (domínio→banco→API): módulo pronto para implementação** | MMS-003-11, MMS-003-13 |
| 2026-08-08 | **MMS-003-05 — Event Storming do Material Requisition criado e aprovado (versão 1.0.0):** 10 comandos CMD-MR + 2 reações automáticas (validação/roteamento), 12 eventos EVT-MR-001..012, 6 políticas, matriz evento×reação, 5 eventos consumidos (MMS-002/004, PR-001, MMS-005) e especificação técnica ADR-010 (exchange `trino.materials`, routing keys `evt.requisition.*`, Outbox, DLQ) — padrão MMS-004-05 | MMS-003-05 |
| 2026-08-08 | **MMS-003-04 — Domain Model do Material Requisition criado e aprovado (versão 1.0.0):** 2 Aggregate Roots (MaterialRequisition com RequisitionItem/RequisitionAttachment; DeliveryLocation), Value Objects (Justification, MasterDataReference, Quantity na unidade base, ApprovalDecision), 13 invariantes INV-MR, factory methods, repositories sem escrita de saldo, 5 domain services (validação/roteamento/projeção de status/aprovação/gerador de código), 7 policies e eventos — padrão MMS-004-04 | MMS-003-04 |
| 2026-08-08 | **MMS-003-03 — State Machine do Material Requisition criada e aprovada (versão 1.0.0):** 9 estados ST-MR-001..009 (Rascunho, Submetida, Em Aprovação, Aprovada, Em Atendimento, Aguardando Compra, Concluída, Rejeitada, Cancelada) com entry/exit, guards, side effects, responsável e auditoria; fluxos principais e alternativos (retorno, aprovação parcial, cancelamento, reserva vencida); matriz de transições e de editabilidade por estado; eventos de domínio e rastreabilidade MR-BR — derivada de MMS-001 §9/§15.1 e das Business Rules, padrão MMS-004-03 | MMS-003-03 |
| 2026-08-08 | **MMS-003-02 — Business Rules do Material Requisition criadas e aprovadas (versão 1.0.0):** 30 regras MR-BR-001..091 em 9 famílias (identidade/cadastro; itens com tamanho e anexos; ciclo de vida; workflow/aprovação com aprovação parcial por item e SoD; validação de estoque após aprovação, rota mista e rastreabilidade bidirecional com PR-001; atendimento/entrega/conclusão; locais de entrega; visão do almoxarifado; auditoria/timeline/segurança), com erros MR-ERR, eventos, config `materials.requisition.*` e rastreabilidade 100% — abre o pacote detalhado do MMS-003, padrão MMS-002-02/MMS-004-02 | MMS-003-02 |
| 2026-08-08 | **MMS-004-13 — API do Inventory Management criada e aprovada (versão 1.0.0):** contrato REST `/api/v1/inventory` com 31 endpoints mapeados a UC-IV-001..012, permissões IV-PERM e eventos EVT-IV-001..019; modelos de recursos (StockBalance como projeção somente leitura, StockMovement com captura de conversão ADR-013, Reservation, Adjustment, InventoryCount, ReplenishmentSuggestion ADR-014, Location); catálogo de erros IV-ERR, segurança (saldo protegido, segregação de funções, idempotência de confirmação), NFRs e roadmap — padrão MMS-002-13. Completa a camada técnica (Database + API) do pacote MMS-004 | MMS-004-13 |
| 2026-08-08 | **MMS-004-11 — Database Model do Inventory Management criado e aprovado (versão 1.0.0):** DDL PostgreSQL completo no schema `materials` (ADR-009) — 10 tabelas (location, stock_movement + line, reservation, adjustment + line, inventory_count + entry, replenishment_suggestion, stock_balance como projeção não editável com `available_qty` gerada), constraints refletindo invariantes INV-IV, conversão de UoM em `stock_movement_line` (ADR-013), sugestão de reposição (ADR-014), FKs físicas intra-módulo + lógicas para Foundation/MMS-002/003/PR-001, triggers de imutabilidade e proteção da projeção de saldo, multiempresa, soft delete, keyset, migração e backup — padrão MMS-002-11. Preenche a maior lacuna do pacote MMS-004 (avança a camada Database da metodologia TPE) | MMS-004-11 |
| 2026-08-08 | **Aprofundamento do MMS-004 nos docs detalhados (ADR-013/014):** **MMS-004-04 Domain Model → 1.1.0** (captura de conversão na MovementLine + novo AR ReplenishmentSuggestion, 6 agregados), **MMS-004-05 Event Storming → 1.1.0** (CMD-IV-021/022, EVT-IV-017..019, total 19 eventos; nota de conversão), **MMS-004-03 State Machine → 1.1.0** (5ª entidade Sugestão de Reposição, ST-IV-040..042) e **MMS-004-07 Use Cases → 1.1.0** (UC-IV-012 Gerenciar Sugestões de Reposição; conversão nos UCs de movimentação). MMS-004 agora reflete ADR-013/014 em visão, regras, estados, domínio, eventos e casos de uso | MMS-004-03, MMS-004-04, MMS-004-05, MMS-004-07 |
| 2026-08-08 | **Propagação de ADR-013 ao MMS-005 → 1.1.0 (visão):** a conferência de recebimento pode ser feita na unidade de compra do item, convertida para a unidade base ao gerar a entrada no estoque (fator aplicado registrado — MMS-004 IV-BR-009); escopo, integração com MMS-002, entradas e histórico atualizados. Fronteira preservada (saldo só no MMS-004, na base). Conclui a largura da propagação de conversão de UoM na suíte MMS (MMS-002/004/005) | MMS-005 |
| 2026-08-08 | **Propagação dos ADRs ao MMS-004 → 1.1.0 (visão + regras):** **MMS-004 README → 1.1.0** e **MMS-004-02 Business Rules → 1.1.0**. IV-BR-009 revisada (saldo na unidade base + conversão de UoM na captura de movimentação, fator aplicado registrado — ADR-013), removendo a antiga restrição de "sem conversão no MVP"; nova família Regras de Reposição (Sugestão) com IV-BR-130 (avaliação → sugestão) e IV-BR-131 (sugestão revisável, rota compra→PR-001 / transferência→documento, execução automática opt-in v2.0 — ADR-014); matriz 40→42/42; roadmap do módulo atualizado (fila de sugestões na v1.1, reposição automática na v2.0). Documentos detalhados do MMS-004 (state machine, domain model, event storming, BPMN, use cases) a revisar em passe posterior para o campo de conversão nos movimentos e o fluxo de sugestão | MMS-004, MMS-004-02 |
